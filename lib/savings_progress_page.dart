...
class _SavingsProgressPageState extends State<SavingsProgressPage> {
  ...
  Widget _buildTopPreview(Map<String, dynamic> data) {
    ...
    final double idealProgress = ((data['expected_saved'] as num) / targetAmount).clamp(0.0, 1.0);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('🔍 儲蓄進度預覽', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.brown[700])),
            const SizedBox(height: 12),
            Text('目前進度：${data['progress_ratio']}% (NT\$${saved.toStringAsFixed(0)}/NT\$${targetAmount.toStringAsFixed(0)})'),
            const SizedBox(height: 8),
            ClipRRect(
              borderRadius: BorderRadius.circular(10),
              child: Column(
  ...
  Widget _buildResultCard(Map<String, dynamic> data) {
    final double progress = (data['progress_ratio'] as num).toDouble() / 100.0;
    final int remainingDays = (data['total_days'] as num).toInt() - (data['days_passed'] as num).toInt();
    final double remaining = targetAmount - (data['current_saved'] as num).toDouble();
    final double daily = remainingDays > 0 ? remaining / remainingDays : 0;

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('🎯 你已儲蓄 NT\$${data['current_saved']} / NT\$${targetAmount.toStringAsFixed(0)}（達成 ${data['progress_ratio']}%）'),
        const SizedBox(height: 8),
        LinearProgressIndicator(value: progress, minHeight: 12, backgroundColor: Colors.grey[300]),
        const SizedBox(height: 24),
        Text('📅 預計應達進度為 ${((data['expected_saved'] as num) / targetAmount * 100).toStringAsFixed(1)}%，等於 NT\$${data['expected_saved']}'),
        Text(data['gap_status'] == 'ahead'
            ? '✅ 你已超前 NT\$${data['gap']}'
            : '⚠️ 你已落後 NT\$${data['gap']}'),
        const SizedBox(height: 24),
        Text('📅 剩下 ${remainingDays} 天，尚需儲蓄 NT\$${remaining.toStringAsFixed(0)}'),
